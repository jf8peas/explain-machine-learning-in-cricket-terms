...
None of this happens outside the framework **Director and Executor** already set up — though not as cleanly as `sr_x_rrr` makes it look. `approved_features` and `forbidden_features` bound the *raw* columns this agent starts from, `strike_rate` and `required_run_rate` — but `sr_x_rrr` is a *derived* column, built by combining two approved ones, and neither list was ever written with derived features in mind. That's the exact brief gap that chapter treats as `umpires_call`, not something the Feature Factory gets to assume is fine just because the ingredients were approved. Permutation importance can only prune a candidate that already exists; manufacturing it in the first place is the decision that actually needs sign-off. `encoding_rule` is more straightforward — it's the reason `venue_dummies` has to be fit on the training fold only, since that rule governs the encoding method itself, not which columns get combined into a new one. The Feature Factory is the Team Sheet's "feature iteration" row, running in code — and "iteration" includes asking permission to combine, not just generating and pruning what's already been cleared.
